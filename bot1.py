    ...
    if os.path.exists(FLOWERS_FILE):
        try:
            with open(FLOWERS_FILE, "r", encoding="utf-8") as f:
                flowers = json.load(f)
            print(f"✓ Загружено {len(flowers)} товаров из {FLOWERS_FILE}")
        except Exception as e:
            print(f"❌ Ошибка загрузки {FLOWERS_FILE}: {e}")
            flowers = []
            save_flowers()
    else:
        print(f"⚠️  Файл {FLOWERS_FILE} не найден, создаем пустой каталог")
        flowers = []
...
print(f"=== Инициализация бота завершена ===")
print(f"Загружено товаров: {len(flowers)}")
...
async def show_price_range(msg: Message, state: FSMContext):
    ...
    print(f"Всего товаров в файле: {len(flowers)}")
    ...
    # Показываем найденные товары в отладке
    print("Найденные товары:")
    for i, f in items[:5]:  # Показываем первые 5
    ...
    print(f"Начинаем отправку {len(items)} товаров...")
    
    for idx, f in items:
        try:
            kb = get_quantity_kb(idx)
            print(f"Отправляем товар {idx+1}/{len(items)}: {f['name']}")
            
            # Проверяем, является ли фото локальным файлом
            if f["photo"].startswith("static/images/"):
                # Отправляем локальный файл
                photo_path = f["photo"]
                print(f"  Путь к фото: {photo_path}")
                
                if os.path.exists(photo_path):
                    print(f"  Файл найден, размер: {os.path.getsize(photo_path)} байт")
                ...
                print(f"  ⚠️  Неожиданный формат фото: {f['photo']}")
                # Отправляем по URL или file_id
                sent = await msg.answer_photo(
                    f["photo"],
                    caption=f"{f['emoji']} {f['name']}\nЦена: {f['price']} руб.",
                    reply_markup=kb
                )
                print(f"  ✓ Фото отправлено по ссылке")
            
            product_message_ids.append(sent.message_id)
            successful_sends += 1
            print(f"  ✓ Товар {f['name']} добавлен в список отправленных")
            
        except Exception as e:
            print(f"  ❌ Ошибка при отправке товара {f['name']}: {e}")
            failed_sends += 1
            ...
            try:
                ...
                product_message_ids.append(sent.message_id)
                print(f"  ✓ Текстовое сообщение отправлено вместо фото")
            except Exception as text_error:
                print(f"  ❌ Не удалось отправить даже текстовое сообщение: {text_error}")
    
    print(f"\nИтоги отправки:")
    print(f"  Успешно отправлено: {successful_sends}")
    print(f"  Ошибок: {failed_sends}")
    print(f"  Всего сообщений: {len(product_message_ids)}")
    ...
    if product_message_ids:
        ...
        print(f"✓ Состояние обновлено, переход к следующему этапу")
    else:
        await msg.answer("Произошла ошибка при загрузке товаров. Попробуйте еще раз.")
        await state.clear()
        print(f"❌ Нет отправленных сообщений, состояние сброшено")
...
async def choose_edit(callback: CallbackQuery, state: FSMContext):
    # Проверяем, что это не кнопка удаления или изменения
    if callback.data in ["edit_delete", "edit_change"]:
        return
    ...
    try:
        ...
        kb = InlineKeyboardMarkup(
            inline_keyboard=[
                ...
                [InlineKeyboardButton(text="Изменить", callback_data=f"change_{idx}")]
            ]
        )
        flower = flowers[idx]
        await callback.message.answer(
...
async def delete_flower(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if 0 <= idx < len(flowers):
            ...
            await callback.answer("Неверный индекс товара")
    except (ValueError, IndexError):
...
async def change_flower(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        await callback.answer("Ошибка при изменении товара")
